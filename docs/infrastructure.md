...
| **Capa de datos (Prisma)** | `backend/prisma/schema.prisma` | Define el **datasource** PostgreSQL (`DATABASE_URL`), el **generator** `prisma-client-js` y los modelos persistidos. En uso por el API: **`User`**. El modelo **`Calendar`** es heredado del scaffold LTI (CV) y no pertenece a ConRutina; ver [data-model.md](./data-model.md). El cliente se genera con `prisma generate`. |
