...
**HabitHero** es una aplicación web **SPA (Single Page Application)** escrita en **TypeScript** con **React**. El estado de hábitos y recompensas sigue viviendo en **memoria del cliente** (`useState` en el componente raíz), pero la UI ya puede consumir un **API HTTP propio** en Node: existe un servidor **Express** ([`server/index.ts`](../server/index.ts)) que usa **Prisma** contra **PostgreSQL** y expone, entre otras, **`GET /api/profile`** (usuario con `id = 1`, pensado como perfil “simulado” de sesión). El componente **`UserProfileCard`** en la cabecera obtiene esos datos con `fetch('/api/profile')`.
...
| API HTTP (Node) | `express`, `cors` | **^4.19.2** / **^2.8.6** | Servidor HTTP en [`server/index.ts`](../server/index.ts); CORS hacia el origen de Vite en desarrollo |
...
| **Componentes de dominio** | `src/app/components/*.tsx` | UI de negocio: cabecera, filas de hábito, calendario semanal, tarjetas de estadísticas/recompensas, modales, **`UserProfileCard`** (perfil vía API). |
| **Servidor API** | `server/index.ts` | Express, CORS, instancia única de `PrismaClient`, rutas REST bajo `/api`. |
...
**Persistencia y API:** Prisma + PostgreSQL están cableados en **`server/index.ts`**. Para ver el perfil en la UI hace falta **PostgreSQL accesible** (`DATABASE_URL` correcto), un registro **`User`** con `id = 1`, y **`npm run dev:api`** además de **`npm run dev`**.
...
├── server/
│   └── index.ts             # API Express (Prisma, CORS, rutas /api)
...
| Archivo | Rol |
|---------|-----|
| `package.json` | Scripts, dependencias, metadatos del paquete (`type: "module"` → ESM nativo en Node para scripts). Incluye **`prisma.schema`**: `backend/prisma/schema.prisma`. Scripts **`dev:api`** (API con `tsx watch`). |
| `vite.config.ts` | Plugins (`react`, `tailwindcss`), alias `@` → `./src`, `assetsInclude` para `.svg` y `.csv`. **`server.proxy`**: prefijo `/api` → `http://localhost:3001` (mismo puerto por defecto que el API; ajusta ambos si cambias `API_PORT`). |
...
| `.env` (raíz, no versionar secretos) | Debe incluir al menos **`DATABASE_URL`** para Prisma y el API Node, y para Docker Compose **`POSTGRES_USER`**, **`POSTGRES_PASSWORD`**; opcionalmente **`POSTGRES_DB`** (por defecto `habithero2`), **`POSTGRES_PORT`**. Opcional: **`API_PORT`** (puerto del Express; por defecto **3001**; debe coincidir con el `target` del proxy en `vite.config.ts`). |
...
| `npm run dev:api` | Arranca el **API Express** con **`tsx watch`** sobre `server/index.ts` (recarga al guardar). Requiere **`DATABASE_URL`** y, para el perfil en pantalla, PostgreSQL con un **`User`** `id = 1`. |
...
1. **Localiza el estado:** Casi toda la lógica de hábitos/recompensas está en `App.tsx`. Para datos de servidor, sigue el patrón de **`UserProfileCard`**: `fetch` a rutas bajo **`/api`**, implementadas en **`server/index.ts`** con Prisma. Si la app crece, conviene extraer contexto (`React.createContext`), un store ligero o un cliente HTTP compartido.
...
6. **Persistencia:** En la SPA los datos de hábitos/recompensas siguen en memoria y se pierden al recargar. Los datos de **usuario en BD** ya pueden leerse vía el **API** (`GET /api/profile`). Para más entidades, amplía el esquema Prisma, crea rutas en `server/index.ts` (o divide en routers) y consume desde React con las mismas URLs `/api/...`.
...
|----------|-------------|
| **Código** | [`server/index.ts`](../server/index.ts) — aplicación Express en TypeScript, ESM (`"type": "module"` en `package.json`). |
| **Arranque en desarrollo** | `npm run dev:api` — usa **`tsx watch`** para ejecutar y recargar el servidor al guardar cambios. |
