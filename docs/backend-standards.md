...
- **Errores de dominio/aplicación**: Clases o tipos discriminados cuando aporten valor
- **Capa HTTP**: Capturar, mapear a códigos HTTP y mensajes en español para el cliente
- **Propagación**: `try/catch` en handlers; evitar tragar errores sin log
...
// Error (500)
{
  "error": "Error al leer el usuario"
}
```

**Evolución recomendada** (errores estructurados; `message` en español para el cliente):
...
El campo `code` permanece en inglés (identificador estable); `message` en español.
